...
\institute[shortinst]{
	\inst{1}  University of Ottawa \hspace{0.5cm}
	\inst{2} Université Libre de Bruxelles \hspace{0.5cm}
	\inst{3} Jagiellonian University \and
	\inst{4} Carleton University \hspace{0.5cm}
	\inst{5} Karlsruhe Institute of Technology \hspace{0.5cm}
	\inst{6} Monash University}
\date{ J. ACM 67/4:1-38, 2020 \\ \vspace{0.5cm} Geometry: Combinatorics and Algorithms Seminar\\Shengzhe Wang \\April 21, 2023}
...
\begin{document}
	...
	\begin{frame}
		...
		\begin{center}
			What is the queue-number of a tree?
			\vfill
	...
	\begin{frame}
		\frametitle{Partitions}
		\begin{center}
			\includegraphics<1>[width=0.5\textwidth]{pics/partition_G.pdf}
			\includegraphics<2>[width=0.5\textwidth]{pics/partition_P.pdf}
			\includegraphics<3>[width=0.7\textwidth]{pics/partition_H.pdf}
		\end{center}
	\end{frame}
	...
	\begin{frame}
		...
		\onslide<1->
		{
			\begin{block}{Definition: Partition and Quotient}
				...
				The quotient of $\mathcal{P}$ is a graph, denoted by $G/\mathcal{P}$, where each vertex $v_i$ corresponds to $P_i$. For any two vertices $v_i,v_j$ in $G/\mathcal{P}$, they are connected if and only if some vertex in $P_i$ is connected to some vertex in $P_j$ in graph $G$.
			\end{block}
		}
		\onslide<2->
		{
			\begin{block}{Definition: $H$-partition}
				An $H$-partition of a graph $G$ is a pair $(A,H)$, s.t. $A = \{A_x | x \in V(H)\}$ is a partition of $V(G)$ and $H$ is a graph isomorphic to the quotient $G/A$.
			\end{block}
		}
	\vfill
		\begin{center}
			...
			\includegraphics<1-2>[width=0.55\textwidth]{pics/partition_H.pdf}
			\includegraphics<3>[width=0.55\textwidth]{pics/partition_bad1.pdf}
			\includegraphics<4>[width=0.55\textwidth]{pics/partition_bad2.pdf}
		\end{center}
	\end{frame}
	...
	\begin{frame}
		...
		\end{center}
		\vfill
		\onslide<3>{ \centering Layering is just a path-partition.}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{block}{Definition: Layered width}
			The layered width of a partition $\mathcal{P}$ of a graph $G$ is the minimum integer $\ell$ such that there exists a path-partition (layering) of $G$, s.t. each element in $\mathcal{P}$ has at most $\ell$ vertices in each element of the path-partition.
		\end{block}
		\vfill
		\begin{center}
	...
	\begin{frame}
		...
		\onslide<8>{\centering A partition of layered width 1 where each part is a vertical path.}
	...
	\begin{frame}
		...
		\onslide<2->
		{
			\centering{\footnotesize Note that three vertices on the outer cycle can always be three vertical paths of $T$.}
		}
		\vfill
		\onslide<3->
		{
